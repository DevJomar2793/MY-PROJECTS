...
const filteredEmployees = computed(() => {
  ...
  return summary.value.employee_list.filter(
    (e) =>
      !q ||
...
const deploymentRate = computed(() => {
  ...
  return Math.round(
    (summary.value.total_deployed / summary.value.total_items) * 100,
  );
});
...
const barWidth = (count, list) => Math.round((count / maxCount(list)) * 100);
...
const designationColor = (label) => {
  const map = {
    ...
    "ASSIGNED (COMPANY/STUDENT)":
      "bg-success-subtle text-success border-success",
    SOLD: "bg-danger-subtle text-danger border-danger",
    SCRAP: "bg-secondary-subtle text-secondary border-secondary",
    "For sale at regular price (good item)":
      "bg-warning-subtle text-warning border-warning",
    "For sale at cheap price (not good item)":
      "bg-orange-subtle text-warning border-warning",
    ...
    <div class="row">
      ...
      <main class="col-md-10 ms-sm-auto px-4 py-4">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
          <div>
            <h4 class="fw-bold text-dark mb-1">
              <i class="bi bi-bar-chart-fill text-primary me-2"></i>Reports
              &amp; Analytics
            </h4>
            <p class="text-muted small mb-0">
              Inventory overview based on live database data
            </p>
          </div>
          <div class="d-flex gap-2">
            ...
            <button
              @click="exportCSV"
              class="btn btn-sm btn-success rounded-pill px-3 fw-semibold shadow-sm"
            >
        ...
        <div v-else-if="summary">
          ...
          <div class="card border-0 rounded-4 shadow-sm mb-4 p-3">
            ...
            <div class="d-flex justify-content-between mt-1">
              <small class="text-muted"
                >{{ summary.total_deployed }} deployed</small
              >
              <small class="text-muted"
                >{{ summary.total_undeployed }} available</small
              >
            </div>
          </div>
          ...
          <ul class="nav nav-pills gap-2 mb-4">
            ...
            <li class="nav-item">
              <button
                class="nav-link rounded-pill px-4 fw-semibold"
                :class="
                  activeTab === 'hardware-types' ? 'active' : 'text-muted'
                "
          ...
          <div v-if="activeTab === 'overview'" class="row g-4">
              ...
              <div class="card border-0 rounded-4 shadow-sm h-100">
                <div class="card-header bg-white border-0 pt-4 pb-2 px-4">
                  <h6 class="fw-bold mb-0">
                    <i class="bi bi-tags-fill text-primary me-2"></i>Designation
                    Breakdown
                  </h6>
                </div>
                <div class="card-body px-4 pb-4">
                  <div
                    v-for="(item, i) in designationList"
                    :key="item.label"
                    class="mb-3"
                  >
                    ...
                    <div class="progress rounded-pill" style="height: 7px">
                      <div
                        ...
                        :class="barColor(i)"
                        :style="{
                          width: barWidth(item.count, designationList) + '%',
                        }"
                        role="progressbar"
                      ></div>
                    </div>
            ...
            <div class="col-md-6">
              <div class="card border-0 rounded-4 shadow-sm h-100">
                <div class="card-header bg-white border-0 pt-4 pb-2 px-4">
                  <h6 class="fw-bold mb-0">
                    <i class="bi bi-cpu-fill text-info me-2"></i>Hardware Type
                    Breakdown
                  </h6>
                </div>
                <div class="card-body px-4 pb-4">
                  ...
                  >
                    <div class="d-flex justify-content-between mb-1">
                      ...
                      >
                      <span
                        class="badge rounded-pill px-2 bg-info text-dark"
                        style="font-size: 10px"
                        >{{ item.count }}</span
                      >
                    </div>
              ...
              <div class="card border-0 rounded-4 shadow-sm">
                <div class="card-header bg-white border-0 pt-4 pb-2 px-4">
                  <h6 class="fw-bold mb-0">
                    ...
                    >Hardware Assigned by Department
                  </h6>
                </div>
                <div class="card-body px-4 pb-4">
                  <div class="row g-3">
                    ...
                    >
                      <div
                        class="bg-light rounded-3 p-3 d-flex align-items-center justify-content-between border"
                      >
                        <div>
          ...
          <div v-if="activeTab === 'employees'">
            <div class="card border-0 rounded-4 shadow-sm">
              ...
              >
                <h6 class="fw-bold mb-0">
                  <i class="bi bi-people-fill text-primary me-2"></i>Deployment
                  by Employee
                </h6>
                <div
                ...
                <div class="table-responsive">
                  <table class="table table-hover align-middle mb-0">
                    <thead>
                      <tr>
                        <th
                          class="border-0 text-muted fw-semibold py-3 ps-4"
                          style="background: #f8f9fa"
                        >
                          3-Code
                        </th>
                        <th
                          class="border-0 text-muted fw-semibold py-3"
                          style="background: #f8f9fa"
                        >
                          Employee Name
                        </th>
                        ...
                        <th
                          class="border-0 text-muted fw-semibold py-3 text-center"
                          style="background: #f8f9fa"
                        >
                          # Items
                        </th>
                        <th
                          class="border-0 text-muted fw-semibold py-3 pe-4 text-center"
                          style="background: #f8f9fa"
                        >
                          Details
                        </th>
                    ...
                    <tbody>
                      ...
                      <tr v-for="emp in filteredEmployees" :key="emp.id">
                        ...
                        <td class="py-3 text-center">
                          <span
                            class="badge bg-success-subtle text-success border border-success-subtle rounded-pill px-3 fw-semibold"
                          >
                            {{ emp.hardware_count }}
                          </span>
                        </td>
                    ...
                    <div class="card border-0 rounded-4 shadow-sm p-4 h-100">
                      ...
                      >
                        <div
                          ...
                          :class="barColor(i)"
                          :style="{
                            width: barWidth(item.count, typeList) + '%',
                          }"
                        ></div>
                      </div>
                    </div>
        ...
        <div v-else class="text-center py-5 text-muted">
          ...
          <p class="mt-3">
            Could not load report data. Make sure the backend is running.
          </p>
        </div>
      ...
      >
        <div class="modal-content border-0 rounded-4 overflow-hidden">
          ...
          <div class="modal-body px-4 pb-4">
            ...
            <div class="row g-3 mb-4">
              ...
              <div class="col-md-4">
                <div class="bg-light rounded-3 p-3 border">
                  ...
                  <div class="fw-medium">
                    {{ selectedEmployee.received_date || "—" }}
                  </div>
                </div>
              </div>
              <div class="col-md-4">
                <div class="bg-light rounded-3 p-3 border">
                  ...
                  <div class="fw-bold fs-4 text-primary">
                    {{ selectedEmployee.hardware_count }}
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- Hardware list -->
            <h6
              class="fw-bold text-secondary mb-3 text-uppercase"
              style="font-size: 11px"
            >
              <i class="bi bi-hdd-stack me-1"></i>Assigned Hardware
            </h6>
            <div
              v-if="selectedEmployee.hardware_items.length === 0"
              class="text-muted fst-italic text-center py-3"
            >
              No items assigned.
            </div>
            <div class="table-responsive border rounded-3 overflow-hidden">
              ...
              >
                ...
                <tbody>
                  <tr
                    v-for="hw in selectedEmployee.hardware_items"
                    :key="hw.ckt_item_number"
                  >
                    <td
                      class="ps-3 py-2 text-primary fw-semibold"
                      style="font-size: 12px"
                    >
                      {{ hw.ckt_item_number }}
                    </td>
                    ...
                    <td class="pe-3 py-2 text-muted" style="font-size: 12px">
                      {{ hw.serial_number }}
                    </td>
                    <td class="pe-3 py-2" style="font-size: 12px">
                      <span
                        class="badge rounded-pill px-2 bg-primary-subtle text-primary border border-primary-subtle"
                        >{{ hw.designation || "—" }}</span
                      >
                    </td>
                  </tr>
