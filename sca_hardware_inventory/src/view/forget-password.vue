...
const step = ref(1); // 1 = request email, 2 = enter token + new password
...
const handleRequest = async () => {
  ...
  try {
    ...
    successMsg.value =
      "If that email exists, a password reset token has been generated. Check the developer console for the token (no email server configured).";
    step.value = 2;
  } catch (e) {
  ...
  >
    <div class="w-100" style="max-width: 420px; padding: 0 20px;">
      ...
      <div class="text-center mb-4">
        ...
        <p class="text-white-50 small">Reset your CKT Inventory account password</p>
      </div>

      <!-- Card -->
      <div class="card border-0 rounded-4 shadow-lg" style="background:rgba(255,255,255,0.95);">
        <div class="card-body p-4 p-md-5">

          <!-- Step indicator -->
          <div class="d-flex align-items-center gap-2 mb-4">
            <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold text-white"
              :style="'width:28px;height:28px;font-size:12px;background:' + (step >= 1 ? '#302b63' : '#dee2e6')">1</div>
            <div class="flex-grow-1" style="height:2px;background:#dee2e6;">
              <div :style="'height:100%;background:#302b63;transition:width 0.4s;width:' + (step >= 2 ? '100%' : '0%')"></div>
            </div>
            <div class="rounded-circle d-flex align-items-center justify-content-center fw-bold text-white"
              :style="'width:28px;height:28px;font-size:12px;background:' + (step >= 2 ? '#302b63' : '#dee2e6')">2</div>
          </div>
          ...
          <p class="text-center text-muted small mb-0">
            ...
            <router-link to="/login" class="text-primary fw-semibold text-decoration-none">Back to login</router-link>
          </p>
        </div>
      </div>
