    ...
    return (
      <div className="flex-1 flex flex-col min-h-screen bg-slate-50 items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold text-slate-800">Screen Not Found</h2>
          <p className="text-slate-500">The screen you are looking for does not exist or has been deleted.</p>
          <Link href="/" className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-blue-600 text-white rounded-xl font-medium hover:bg-blue-700 transition shadow-sm border border-blue-700">
      ...
      <main className="p-8 max-w-4xl mx-auto w-full">
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold text-slate-800 mb-2 tracking-tight">
            Screen Details: <span className="text-indigo-600">{screenData.screen_label}</span>
          </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
              ...
              <div className="space-y-6">
                <div className="bg-slate-50/50 rounded-2xl p-6 border border-slate-100 flex items-center justify-between shadow-sm hover:shadow transition-shadow">
                  <div className="flex items-center gap-4 border-l-4 border-blue-500 pl-4">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-slate-800">{screenData.status}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-2 bg-slate-50/50 rounded-2xl p-4 border border-slate-100">
                  <InfoItem icon={<Hash className="w-5 h-5 text-indigo-500" />} label="Alpha" value={screenData.alpha} />
                  <InfoItem icon={<Monitor className="w-5 h-5 text-indigo-500" />} label="Screen Type" value={String(screenData.Screen_type)} />
                  <InfoItem icon={<Hash className="w-5 h-5 text-indigo-500" />} label="Screen Number" value={String(screenData.screen_number)} />
                </div>
              </div>

              {/* Descriptions & Labels */}
              <div className="space-y-2 bg-slate-50/50 rounded-2xl p-4 border border-slate-100 h-fit">
                <InfoItem icon={<Tag className="w-5 h-5 text-purple-500" />} label="Screen Label" value={screenData.screen_label} />
                <InfoItem icon={<FileText className="w-5 h-5 text-purple-500" />} label="File Label" value={screenData.file_label} />
                <InfoItem icon={<AlignLeft className="w-5 h-5 text-purple-500" />} label="Description" value={screenData.screen_description} />
                <InfoItem icon={<Monitor className="w-5 h-5 text-purple-500" />} label="Sitemap" value={screenData.sitemap} />
              </div>
            ...
            <div className="mt-8 pt-8 border-t border-slate-100">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-3">
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Additional Notes</h3>
                  <div className="bg-slate-50 min-h-[100px] p-5 rounded-2xl border border-slate-200/60 text-slate-700 whitespace-pre-wrap shadow-inner leading-relaxed">
                    {screenData.notes || <span className="text-slate-400 italic">No additional notes provided.</span>}
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="flex items-start gap-4 p-3 hover:bg-white rounded-xl transition-colors hover:shadow-sm">
      ...
      <div>
        ...
        <p className="text-[15px] font-medium text-slate-800 leading-tight">{value}</p>
      </div>
    </div>
  );
}
